...
class App extends Component {
  state = {
    ...
    score: 0,
    time: 60,
    targetImageUrl: imagesList[0].imageUrl,
  }
  ...
  clickThumbnail = clickedThumbUrl => {
    const {targetImageUrl} = this.state
    const selected = imagesList.find(
      item => item.thumbnailUrl === clickedThumbUrl,
    )
    if (selected.imageUrl === targetImageUrl) {
      const newImage = imagesList[Math.floor(Math.random() * imagesList.length)]
      this.setState(prevState => ({
        score: prevState.score + 1,
        targetImageUrl: newImage.imageUrl,
      }))
    } else {
      clearInterval(this.timerId)
      this.setState({isGameOver: true})
  ...
  playAgain = () => {
    clearInterval(this.timerId)
    const newTarget = imagesList[Math.floor(Math.random() * imagesList.length)]
    ...
    this.setState({
      ...
      isGameOver: false,
      activeCategory: 'FRUIT',
      targetImageUrl: newTarget.imageUrl,
    })
  ...
  render() {
    const {score, time, activeCategory, isGameOver, targetImageUrl} = this.state
        ...
        <div className="content-div">
          {!isGameOver ? (
            <div className="first-div">
              <img src={targetImageUrl} className="big-image" alt="match" />
                  ...
                  <li key={tab.tabId}>
                    <button
                      ...
                      className={`tab-button ${
                        activeCategory === tab.tabId ? 'highlight-text' : ''
                      }`}
                      onClick={() => this.clickTab(tab.tabId)}
                    >
                      {tab.displayText}
                    </button>
                  </li>
              ...
              <ul className="thumbnail-images">
                {filteredThumbnails.slice(0, 15).map(item => (
                  <li key={item.id}>
                    <button
                      type="button"
                      className="image-button"
                      onClick={() => this.clickThumbnail(item.thumbnailUrl)}
                    >
                      <img
